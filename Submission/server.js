...
// ****************************************************************************************
// ****************************************************************************************
// Need to add specific functions for file loading and saving
// ****************************************************************************************
// ****************************************************************************************
// ****************************************************************************************

// Including YAML component
...
let database = {
  ...
  articles: {},
  nextArticleId: 1,
  comments: {},   // id, "body", "username", articleId, [upvotedBy], [downvotedBy]
  nextCommentId: 1
};
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body && requestComment.articleId &&
    ...
    const comment = {
      ...
      body: requestComment.body.toString(),
    ...
    response.body = {comment: comment};
    //saveDatabase(comment, 'create');

    //let testDB = loadDatabase();
    //console.log('testDB: ' + JSON.stringify(testDB));
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    ...
    //saveDatabase(savedComment.body);
    response.status = 200;
  }
...
function deleteComment(url, request) {
  ...
  //console.log('savedComment: ' + JSON.stringify(savedComment));
  //console.log('database.comments[id]: ' + JSON.stringify(database.comments[id]));
...
/*function saveDatabase(object, process) {
  const dataFile = yaml.read('file.yaml');
  console.log('dataFile: ' + JSON.stringify(dataFile));
  //yaml.write('file.yaml', object);
}

function loadDatabase() {
  const object = yaml.read('file.yaml');
  if (yaml.read('file.yaml')) {
